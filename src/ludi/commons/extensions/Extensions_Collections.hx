...
class Extensions_Collections {
	...
	public static function mapLength<K, V>(map:Map<K,V>):Int {
		var i = 0;
	...
	public static function toGrid<T>(arr:Array<T>, mapper:(T) -> IVec2):GridMap<T> {
	...
	public static function getsertLazy<T>(map:Map<String, T>, key:String, def:Void -> T):T {
		if (map.exists(key)) {
			return map.get(key);
		}
		var d = def();
	...
	static public function filterValues<K, V>(map: Map<K, V>, filter: (K, V) -> Bool): Array<V> {
	...
	static public function computeInt<T>(arr:Array<T>, compute:(T, Int) -> Int):Int {
		var currentValue = 0;
		for (eachItem in arr) {
	...
	static public function verticalCut<T>(arr:Array<Array<T>>, idx:Int):Array<T> {
		var result:Array<T> = [];
		for (subArr in arr) {
